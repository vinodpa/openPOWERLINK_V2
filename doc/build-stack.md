...
- **CFG_COMPILE_LIB_MN_DRV_DUALPROCSHM**
  ...
  memory library. The library consists of the kernel part of the stack using CAL modules
...
- **CFG_COMPILE_LIB_MN_APP_DUALPROCSHM**
  ...
  Compile the user part of the openPOWERLINK MN library using dual processor shared
  memory library. The library consists of the user part of the stack using CAL modules
  for the dual processor library. The library can be linked with a host application
  designed to run on the ARM processor as the host processor. It requires the kernel
  daemon to run on separate processor (PCP) sharing a part of the memory.
    - __CFG_COMPILE_LIB_MN_HW_LIB_DIR__
